...
public class ZirkelApplet extends javax.swing.JApplet
...
//        On firefox mac, I don't know why, but getParameter("style") return null
//        even if there is a "style" parameter tag... so :
        if (style==null) {
            if ((getParameter("tools")!=null)||(getParameter("options")!=null)) style="icons";
                    else style="plain";
        }
...
//			IB.addSeparatorLeft();
        ...
        if (style.equals("plain")) getContentPane().add("Center",ZC);
        else getContentPane().add("Center",new Panel3D(ZC,ZC.getBackground()));
        ...
        try {
            Global.setParameter("minlinesize",
                    new Double(getParameter("minlinesize")).doubleValue());
        } catch (Exception e) {}
        try {
        ...
        if (breaks) {
            ...
            javax.swing.JPanel pic=new Panel3D(IC);
...
//        IA.forceRepaint();
        ...
        ZC.updateMacroBar();
        ZC.recompute();
//        Very weird thing which cause applet to be displayed on firefox mac :
