 ...
 */
class BasicIcon extends JPanel
 ...
 */
class OverflowButton extends JPanel
{	IconBar IB;
	boolean Left=true;
...
public class IconBar extends javax.swing.JPanel
...
{	Vector Left=new Vector(),Right=new Vector();
	...
	{	this(f,true);
        ...
        public void add(BasicIcon i){
           java.net.URL myurl=getClass().getResource("/eric/icons/palette/"+i.Name+".png");
           if (myurl!=null) {
               super.add(i);
           }else{
               W-=i.width()+Offset;
	...
	*/
	...
	{	if (F==null || WHelp!=null) return;
        if (rene.zirkel.Zirkel.IsApplet) return;
		Point P=i.getPosition();
		WHelp=new JDialog(F);
        WHelp.setAlwaysOnTop(true);
        WHelp.setDefaultCloseOperation(javax.swing.WindowConstants.DO_NOTHING_ON_CLOSE);
	...
	*/
	...
	{	CloseFrame f=new CloseFrame("Test");
		...
		f.add("Center",new IconBarPanel(
			new Panel3D(IA),new Panel3D(new JPanel())));
		f.pack();	
		f.center();
