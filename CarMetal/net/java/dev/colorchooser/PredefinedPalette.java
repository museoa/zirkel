...
class PredefinedPalette extends Palette {
    ...
    Dimension calcSize() {
        ...
        int perRow=(int) Math.round(Math.sqrt(count));
        int perCol=(perRow*perRow>=count)?perRow:perRow+1;
        int width = perCol * (swatchSize + gap) + gap;
        int height = perRow * (swatchSize + gap) + gap;
